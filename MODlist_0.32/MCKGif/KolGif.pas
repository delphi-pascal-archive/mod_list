...
//{$DEFINE USE_ANIMATION_OBJ}
// If you plan to use not only TGifShow, but instead of it want to use
// TAniShow and animate other objects derived from TAnimation, add this
// symbol to your project options. In such CASE do not use TGifShow at all
// but replace it with TAniShow everywhere in the code.

uses windows, KOL {, ChkGdi} {$IFDEF USE_ANIMATION_OBJ}, Animation {$ENDIF}
     {$IFDEF USE_KOLGRAPHIC} , KOLGraphicCompression {$ENDIF};
...
/////////////////////////////////////////////////////////////
  PGif = ^TGif;
  TGif = object( {$IFDEF USE_ANIMATION_OBJ} TAnimation {$ELSE} TObj {$ENDIF} )
  ...
  protected
    {$IFNDEF USE_ANIMATION_OBJ}
    FOnChanged: TOnEvent;
    {$ENDIF}
    function GetBkColor: TColor; {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
    procedure SetBkColor(Value: TColor); {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
    function GetWidth : Integer; {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
    procedure SetWidth( Value : Integer ); {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
    function GetHeight : Integer; {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
    procedure SetHeight( Value : Integer ); {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
    function GetFrame : Integer; {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
    procedure SetFrame( Value : Integer ); {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
    function GetFrameCount : Integer; {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
    function GetDelays( Idx : Integer ) : Integer; {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
    procedure SetDelays( Idx : Integer; Value : Integer ); {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
    procedure Changed; {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
    function GetTransparent: Boolean; {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
  public
    destructor Destroy; virtual;
    {* Destructor. }
    procedure Clear; {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
    {* Obvious. }
    procedure FreeResources; {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
    {* Call this method to free GDI resources, allocated for decoding gif image,
       ...
       a bit. }
    procedure Draw( DC : HDC; X, Y : Integer ); {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
    {* Draws current frame. }
    procedure DrawTransp( DC: HDC; X, Y: Integer ); {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
    ...
    procedure DrawTransparent( DC : HDC; X, Y : Integer; TranspColor : TColor );
      {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
    {* Draws current frame transparently. }
    procedure StretchDraw( DC : HDC; const Dest : TRect ); {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
    {* Draws current frame with stretching. }
    procedure StretchDrawTransp( DC: HDC; const Dest: TRect ); {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
    ...
    function LoadFromStream( Stream : PStream ) : Boolean; {$IFDEF USE_ANIMATION_OBJ} virtual; {$ENDIF}
...
{$IFDEF USE_KOLGRAPHIC}
procedure ReadGIFData(Stream: PStream; var Header: TBitmapInfoHeader;
  Interlaced: Boolean; IntBitPerPixel: Byte; Data: Pointer;
  var Corrupted: Boolean);
var {$IFDEF NOCLASSES} Decoder: PGIFLZWDecoder; {$ELSE} Decoder: TGIFLZWDecoder; {$ENDIF}
    InitCodeSize: Byte;
    Marker: DWORD;
    Pass, Increment: Integer;
    RawData, Run: PByte;
    TargetBuffer, TargetRun, Line: Pointer;
    I, BytesPerRow: Integer;
begin
  Corrupted := FALSE;
  ...
  GetMem(TargetBuffer,Header.biWidth*(Header.biHeight+1));
  TRY
    ...
    until Increment=0;
    {$IFDEF NOCLASSES}
    new( Decoder, Create(InitCodeSize) );
    {$ELSE} Decoder:=TGIFLZWDecoder.Create(InitCodeSize); {$ENDIF}
    try
      Run:=RawData;
      Decoder.Decode(Pointer(Run),TargetBuffer,Pass,Header.biWidth*Header.biHeight);
    finally
      Decoder.Free;
    end;
    // finally transfer image data
    BytesPerRow := (Header.biWidth + 3) and not 3;
    if not Interlaced then
    begin
      TargetRun:=TargetBuffer;
      for I:=0 to Header.biHeight-1 do
      begin
        Line:=Pointer( Integer( Data ) + (Header.biHeight - I - 1) * BytesPerRow );
        Move(TargetRun^,Line^,Header.biWidth);
        Inc(PByte(TargetRun),Header.biWidth);
      end;
    end
      else
    begin
...
var Mem : PStream;
  ...
  begin
    ...
    with BIH do begin
      ...
      biSizeImage := (((biWidth *
        {$IFDEF USE_KOLGRAPHIC} 8 {$ELSE} biBitCount {$ENDIF}
        + 31) div 32) * 4) * biHeight;
      biXPelsPerMeter := 0;
      biYPelsPerMeter := 0;
      biClrUsed := 0;
      biClrImportant := 0;
...
begin
  ...
  begin
    ...
    {$ENDIF}
    {$IFDEF GIF_DEBUG}
    Mem.SaveToFile( GetStartDir + Int2Str( FFrameIndex ) + 'bitmap.bmp', 0, Mem.Size );
    {$ENDIF}
...
var
  ...
  begin
    ...
    FGifData.DBitsPerPixel := 1 +
      (ScreenDesc.PackedFields and LSD_COLOR_TABLE_SIZE);
    FGifData.DColorResBits := 1 +
      (ScreenDesc.PackedFields and LSD_COLOR_RESOLUTION) shr 4;
  end;
    ...
    //try
      ...
      begin
        ...
        while not (SeparatorChar in [CHR_TRAILER, #0]) and not
          (Stream.Position >= Stream.Size) and (DWORD(OldPos) <> Stream.Position) do
        begin
          ...
          begin
            //try
              ...
              //try
                ...
                Extensions := nil;
                if not NewItem.LoadFromStream(Stream) then
                begin
                  NewItem.Free;
                  Exit;
                end;
...
begin
  ...
  while FCurIndex < Min( Frame, Count ) do
  begin
    ...
    if (DM = dmUndefined) and (FCurIndex < Count-1) and Frames[ FCurIndex + 1 ].Transparent then
       DM := dmLeave;
    case DM of
    dmRestoreBackground:
      begin
    ...
    begin
      ...
      begin
         {$IFDEF GIF_DEBUG}
         if F.Mask <> nil then
           F.Mask.SaveToFile( GetStartDir + Int2Str( FCurIndex ) + 'Msk.bmp' );
         {$ENDIF}
         DrawCurFrameMask;
         {$IFDEF GIF_DEBUG}
...
procedure TGif.SetBkColor(Value: TColor);
begin
  //FBkColor := Value;
