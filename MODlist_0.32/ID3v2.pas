...
type
  ...
  TDuplicateFrameMethod = (dfmFirst, dfmFirstNonEmpty, dfmIgnoreDuplicate);
{        dfmFirst - добавляется только один тег, последующие игнорируются
  ...
  private
    ...
    function  _AddTagEx(AID: TID; AFlags: Word = 0): Integer;                       // Добавляет тег
...
begin
    ...
    Result.FDuplicateFrameMethod:= dfmFirstNonEmpty;
    Result.ResetData;
end;
...
const
  ...
  ID3V2_Frames: array [TTagsOfID3] of record
                                  Old: array[1..3] of Char; //(ID3v2.2.x)
...
type
  TFrameHeader = packed record
    case Boolean of
      ...
      False: (IDOld: array [1..3] of Char;                                      { Frame ID }
    ...
    try
      ...
      try
        ...
        while (SourceFile.Position < TagSize) and (SourceFile.Position < SourceFile.Size) do begin
          ...
          if IsNew then DataSize := Swap32(Frame.SizeNew)
          else DataSize := Frame.SizeOld[1] shl 16 + Frame.SizeOld[2] shl 8 + Frame.SizeOld[3];
...
begin
    ...
    try
      ...
      if DeleteFile(PChar(FileName)) and MoveFile(PChar(BufferName), PChar(FileName)) then Result := true;
    except
...
begin
    DisposeMem(Tags[Index].Data);
    FreeMem(FTagList.Items[Index]);
...
begin
    ...
    if Frame <> nil then begin
      if Frame.Size > 0 then
        SetLength(S, Frame.Size);
        Move(Frame.Data^, S[1], Frame.Size);
        ...
        if Index = ID3V2_Frames[tiComment].New then begin
          while (Char(WC^) <> #0)and(Len > 0)  do begin inc(WC); dec(Len); end;
          while (Char(WC^) = #0) and(Len > 0) do begin inc(WC); dec(Len); end;
        end;
...
begin
    ...
    if Index=tiGenre then begin //correct '(NN)Genre' to 'Genre'
      i:= Pos(')', Result);
      if i>0 then Delete(Result, 1, i);
    end;
    repeat                      //correct #0'URL' to 'URL'
      i:= Pos(#0, Result);
